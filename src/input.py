...
class InputRecords(object):
    ...
    def get_keymapping(self):
        ...
        k2type = {}
        k2defaultv = {}
        sql = 'SHOW FULL COLUMNS FROM `%s`' % self.DB_TABLE
    ...
    def to_db(self):
        ...
        str_keys = ','.join(keys)
        sql = 'INSERT INTO `%s`(%s) VALUES (%s)' % (self.DB_TABLE, str_keys, ','.join(['%s' for i in range(len(keys))]))
